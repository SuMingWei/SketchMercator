...
class fcmIteration : public sketchIterationTemplate {
public:

    map <flowkey_t, int> packetMap; // this is for gt
    map <flowkey_t, int> flowkey_tracking; // this is heavy flowkey tracking
    map <flowkey_t, int> flowkey_topk; // this is heavy flowkey tracking

    int reg_val_all[4096];
    int hash_table_key[4096];
    int hash_table_value[4096];

    HashSeedSet *sampling_hash;
    vector<sketchTemplate> fcm_level;
    fcmIteration();
