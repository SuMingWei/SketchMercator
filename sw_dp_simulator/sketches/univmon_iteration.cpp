#include "univmon_iteration.h"

univmonIteration::univmonIteration()
{
}

void univmonIteration::init(parameters &params)
{
    sampling_hash = new HashSeedSet(25);
    for(int level=0; level<25; level++) {
        countSketch_level.push_back(sketchTemplate(params, level));
...
void univmonIteration::load_hash(parameters &params) {
    load_hash_seeds(*sampling_hash, countSketch_level, params);
    for(int i = 0; i < 25; i++) {
        if (params.is_same_level_hash == 1) {
            countSketch_level[i].index_hash = countSketch_level[0].index_hash;
            countSketch_level[i].res_hash = countSketch_level[0].res_hash;
        }
    }
}

void univmonIteration::iteration(packet_summary &p, parameters &params)
{
    flowkey_t flowkey;
//        print_packet(*p);
...
//        print_flowkey(flowkey);
    ...
    if (params.is_update_last_level == 1) {
        if(last_level < params.level) {
            estimate = countSketch_level[last_level].count_sketch(flowkey, params, elem);
        ...
        for (int level = 0; level <= last_level; level++) {
            if (level >= params.level)
                break;
            estimate = countSketch_level[level].count_sketch(flowkey, params, elem);
...
void univmonIteration::file_print(parameters &params)
{
    univmon_file_print(countSketch_level, level_PQ_top200, *sampling_hash, params, packetMap, flowkey_tracking, 0);
}

void univmonIteration::clear(parameters &params)
{
    for(int i = 0; i < 25; i++) {
        countSketch_level[i].clear();
