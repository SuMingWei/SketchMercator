...
int main(int argc, char* argv[]){
    char* pcap_path_origin = (char*)"/home/ming/SketchMercator/traffic_sampler/syn.pcap";
    char* pcap_path_sorted = (char*)"/home/ming/SketchMercator/traffic_sampler/sorted_syn.pcap";
    string flowkey = "srcIP";

    /* summary flow info */
    map<Flowkey_t, int> flow_stream_origin;
    map<Flowkey_t, int> flow_stream_sorted;
    pcap_parse(pcap_path_origin, flow_stream_origin, flowkey);
    pcap_parse(pcap_path_sorted, flow_stream_sorted, flowkey);

    /* get flow size distribution */
    map<int, int> flow_size_dist_origin;
    map<int, int> flow_size_dist_sorted;
    for(auto flow : flow_stream_origin){
        flow_size_dist_origin[flow.second]++;
    }
    for(auto flow : flow_stream_sorted){
        flow_size_dist_sorted[flow.second]++;
    }

    /* write flow size distribution to txt file */
    ofstream outFile_origin("./fs_dist/verified_origin.txt");
    ofstream outFile_sorted("./fs_dist/verified_sorted.txt");
    for(auto item: flow_size_dist_origin){
        outFile_origin << item.first << " " << item.second << endl;
    }
    for(auto item: flow_size_dist_sorted){
        outFile_sorted << item.first << " " << item.second << endl;
    }
