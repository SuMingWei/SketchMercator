...
for i in range(0, array_size, samping_size):
    ip_list.append(long2ip(i))
...
pcap_file_name = "test_%d_%d_%dms_us.pcap" % (array_size, samping_size, total_ms)
print(pcap_file_name)

for ms in range(0, total_ms+1):
    if ms % 10 == 0:
        print("%dms %s" % ( ms, timer.lap_10_milli_string()))
    for us in range(0, 1000, 100):
