...
for (pcap_full_path, pcap_folder_path, pcap_file_name) in ret_list:
    ...
    cmd_template_1 =  "%s/main -m -f %s -n %s -r 'cs' -p 1 -k 'srcIP' "
    cmd_1 = cmd_template_1  % (sw_dp_simulator_path, pcap_full_path, pcap_file_name)

    cmd_template_2 =  "-w %d -d 4 -l 1 -c -e %d -b %d -a %s -z '%s' -g %s -o %s"
    log_template = "[%s] " % (CS_NAME) + "width(%d)_epoch(%d)_P(%d)_delay(%s) " + pcap_file_name
    ...
    for epoch in [1, 5, 10, 20, 30]:
        for width in [1024, 2048, 4096, 8192, 16384]:
            ...
            output_dir = sw_dp_path_input(CS_NAME, epoch, width, d=4, l=1, problem=0, crc=True, pcap_file_name=pcap_file_name, hardware=True, api=API)
            path = get_pcounter_path(CS_NAME, API, width, epoch)
