...
SCRIPT_PATH_CPP = "$sketch_home/tofino_control_plane/cpp/SketchMD"
...
def turn_on_switch_cpp_internal(tuple):
    tofino_cmd = "%s/ssh.sh" % (SCRIPT_PATH_CPP) + " %s %s %s" % tuple
...
SCRIPT_PATH_PYTHON = "$sketch_home/tofino_control_plane/python/SketchMD"
...
def python_setup(tuple, caida_date):
    tofino_cmd = "%s/ssh_setup.sh" % SCRIPT_PATH_PYTHON + " %s %s %s " % tuple + caida_date
    cmd = "ssh %s \'%s\'" % (TOFINO_STR, tofino_cmd)
    print("[cmd - %s]\n" % cmd)
    os.system(cmd)
###
