...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < qcount; i++) {
        ...
        try {
            ...
            for(auto it = query.m_select.begin(); it != query.m_select.end(); it++) {
                printf("('%s', ", (*it)->get_name());
                for(auto jt = query.m_group_by.m_terms.begin(); jt != query.m_group_by.m_terms.end(); jt++) {
                    if(jt == query.m_group_by.m_terms.end() - 1)
                        printf("'%s')\n", jt->m_op->get_name());
                    else
                        printf("'%s', ", jt->m_op->get_name());
                }
            }
            
            //printf("%s\n", query.m_select[0]->get_name());
            //printf("%s\n", query.m_group_by.m_terms[0].m_op->get_name());
