...
void CM::init(parameters &params)
{
    cout << "[CPP] [CM] init" << endl;
    if (strcmp(params.str_args_1, "pingpong") == 0) {
        sprintf(params.program_name, "p416_cm_pp");
    }
    else {
        sprintf(params.program_name, "p416_cm");
    }
...
void CM::table_init(parameters &params)
{
    cout << "[CPP] [CM] table init" << endl;
...
void CM::setup_pingpong(parameters &params, int e_count)
{
    if (isPrint)
        cout << "[CPP] [CM] setup_pingpong" << endl;
    if (e_count % 2 == 1) {
...
void CM::test(parameters &params)
{
    cout << "[CPP] CM test" << endl;
