...
void write_point_two(const bfrt::BfRtTable *register_table, bf_rt_target_t &dev_tgt, char* data_field_name, bf_rt_id_t &reg_index_key_id, uint64_t key, uint64_t value1, uint64_t value2)
{
    char data_field_name_first[5000];
    sprintf(data_field_name_first, "%s.first", data_field_name);
    char data_field_name_second[5000];
    sprintf(data_field_name_second, "%s.second", data_field_name);

    std::unique_ptr<bfrt::BfRtTableKey> register_table_key;
    auto bf_status = register_table->keyAllocate(&register_table_key);
    assert(bf_status == BF_SUCCESS);
    bf_status = register_table_key->setValue(reg_index_key_id, key);
    assert(bf_status == BF_SUCCESS);

    bf_rt_id_t data_id_first;
    bf_status = register_table->dataFieldIdGet(data_field_name_first, &data_id_first);
    assert(bf_status == BF_SUCCESS);

    bf_rt_id_t data_id_second;
    bf_status = register_table->dataFieldIdGet(data_field_name_second, &data_id_second);
    assert(bf_status == BF_SUCCESS);
    ...
    bf_status = register_table_data->setValue(data_id_first, value1);
    assert(bf_status == BF_SUCCESS);

    bf_status = register_table_data->setValue(data_id_second, value2);
    assert(bf_status == BF_SUCCESS);
