...
class UnivMon(Connection):

    def __init__(self, args):
        if args.mode == "pingpong":
            self.program_name = "p416_univmon_pp"
        else:
            self.program_name = "p416_univmon"

        print(self.program_name)
        self.is_simulator = args.simulator
        super(UnivMon, self).__init__(self.program_name, self.is_simulator)

    def configure(self, args):
        # print("[PYTHON] [UnivMon] configure")
        self.tcam = TCAM(self.gc, self.bfrt_info)
        self.tcam.setup_univmon()

    def table_setup(self, args):
        # args has only information about sketch_name / mode
        # setup is only necessary for sol3.
        print("[PYTHON] [UnivMon] table_setup")
        super(UnivMon, self).table_setup()
        if args.mode != "pingpong":
            self.table_dict = {}
            for array_size in [32768]:
                table_list = []
    ...
    def read(self, params):
        print("[PYTHON] [UnivMon] read - not implemented")

    def reset(self, params): # bulk reset, only called by sol3
        print("[PYTHON] [UnivMon] reset")
    ...
    def clear(self, args, params): # baseline, pingpong, noreset, sol3
        print("[PYTHON] [UnivMon] clear")
        super(UnivMon, self).clear(args, params)
