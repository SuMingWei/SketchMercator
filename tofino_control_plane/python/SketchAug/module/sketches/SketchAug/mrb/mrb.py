...
class MRB(Connection):

    def __init__(self, args):
        if args.mode == "pingpong":
            self.program_name = "p416_mrb_pp"
        else:
            self.program_name = "p416_mrb"

        print(self.program_name)
        self.is_simulator = args.simulator
        super(MRB, self).__init__(self.program_name, self.is_simulator)

    def configure(self, args):
        # print("[PYTHON] [MRB] configure")
    ...
    def table_setup(self, args):
        ...
        print("[PYTHON] [MRB] table_setup")
        super(MRB, self).table_setup()
        if args.mode != "pingpong":
            self.table_dict = {}
    ...
    def read(self, params):
        print("[PYTHON] [MRB] read - not implemented")

    def reset(self, params): # bulk reset, only called by sol3
        print("[PYTHON] [MRB] reset")
    ...
    def clear(self, args, params): # baseline, pingpong, noreset, sol3
        print("[PYTHON] [MRB] clear")
        super(MRB, self).clear(args, params)
