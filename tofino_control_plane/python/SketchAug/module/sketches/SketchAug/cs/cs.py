...
class CS(Connection):
    ...
    def __init__(self, args):
        if args.mode == "pingpong":
            if args.array_size == 16384:
                self.program_name = "p416_cs_pp"
            else: 
                self.program_name = "p416_cs_pp_small"
        else:
            self.program_name = "p416_cs"

        print(self.program_name)
        self.is_simulator = args.simulator
        super(CS, self).__init__(self.program_name, self.is_simulator)

    def configure(self, args):
        # print("[PYTHON] [CS] configure")
        pass

    def table_setup(self, args):
        # args has only information about sketch_name / mode
        # setup is only necessary for sol3.
        print("[PYTHON] [CS] table_setup")
        super(CS, self).table_setup()
        if args.mode != "pingpong":
            self.table_dict = {}
    ...
    def read(self, params):
        print("[PYTHON] [CS] read - not implemented")

    def reset(self, params): # bulk reset, only called by sol3
        # print("[PYTHON] [CS] reset")
        ...
        for table in self.table_dict[array_size]:
            ...
            # print("pp")
            # table.entry_del(self.gc.Target(device_id=0, pipe_id=0xffff))
            # table.clear()

    def clear(self, args, params): # baseline, pingpong, noreset, sol3
        print("[PYTHON] [CS] clear")
        super(CS, self).clear(args, params)
