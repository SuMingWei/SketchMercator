...
for epoch in [1,3,5]:
    ...
    for width in [256, 512, 1024, 2048, 4096]:
        ...
        for pcap_file_name in pcap_file_name_list:
            cp_output_path = get_sketch_cp_mrb_path(hash_set_num=1, w=width, d=row, l=level, sketch_aug=False, crc=True, epoch=epoch, pcap_file_name=pcap_file_name)
            saver = PklSaver(cp_output_path, "data.pkl")
            load_data = saver.load()
            data_normal += load_data

            cp_output_path = get_sketch_cp_mrb_path(hash_set_num=1, w=width, d=row, l=level, sketch_aug=True, crc=True, epoch=epoch, pcap_file_name=pcap_file_name)
            saver = PklSaver(cp_output_path, "data.pkl")
            load_data = saver.load()
            data_aug += load_data

        metric = (median(data_aug)-median(data_normal)) / median(data_normal) * 100
        print("epoch(%d) width(%d) %.2f - %.2f = %.2f" % (epoch, width, median(data_aug), median(data_normal), median(data_aug) -median(data_normal)))
        # print("min(%.2f) max(%.2f) min(%.2f) max(%.2f)" % (min(data_aug), max(data_aug), min(data_normal), max(data_normal)))
