    ...
    for file_name in pcap_file_name_list:
        ...
        try:
            ...
            for (wrong_count, wrong_sum, total_array_size, true, sim, sim_error, tofino, tofino_error, sim_ARE_error, tofino_ARE_error) in load_data:
                ...
                global_sim_error_list_dict[epoch].append(sim_error)
...
for epoch in [1, 5, 10, 20, 30]:
    global_sim_error_list_dict[epoch] = []
...
for mode in mode_list:
    ...
    # print("sim_error(%.2f)" % median(global_sim_error_list_dict[epoch]))
    print()
    print()
...
x_label = ["1s", "5s", "10s", "20s", "30s"]
expected = []
for epoch in [1, 5, 10, 20, 30]:
    expected.append(median(global_sim_error_list_dict[epoch]))


print("expected:", expected)
print("unopt:", data["baseline"])
...
ax.set_xlabel('epoch length', fontsize=14)
plt.tick_params(labelsize=12)
plt.grid(color='gray', linestyle='--', linewidth=1, axis='y')
# plt.xticks(x_label)
ax.set_title('CS (array size 64K)', fontsize=13)
...
fig.tight_layout()
plt.savefig("cs_epoch_length_re.pdf")
# plt.savefig("/Users/hnamkung/Desktop/mrb.png")
plt.show()



print(data_wrong_sum["baseline"])
print(data_wrong_sum["pingpong"])
print(data_wrong_sum["noreset"])
print(data_wrong_sum["sol3cpp"])
...
ax.plot(x_label, data_wrong_sum["pingpong"], label='Sol 1', color="C4", marker=markerst2, markersize=markersize, linestyle=linest)
...
# plt.legend(loc="upper right", fontsize=9)
fig.tight_layout()
plt.savefig("cs_epoch_length_diff.pdf")
# plt.savefig("/Users/hnamkung/Desktop/mrb.png")
plt.show()
