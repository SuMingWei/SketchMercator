...
def count_sketch_plot(data, title, labels, colors, figsize):
    ...
    if title == "univmon-cardinality":
        plt.title("UnivMon Cardinality", fontsize = 14)
        from matplotlib.ticker import MultipleLocator
    ...
    if title == "univmon-entropy":
        plt.title("UnivMon Entropy", fontsize = 14)
        plt.ylabel("RE ($\%$)", fontsize = 14)
        # plt.ylim([0, 12])
    #     plt.xticks([0.5, 2.5, 4.5, 6.5, 8.5],
    #                ["trace1",
    ...
    plt.tight_layout()
    if title == "univmon-cardinality":
        plt.savefig("/Users/hnamkung/Desktop/univmon-cardinality.pdf")
    if title == "univmon-entropy":
        plt.savefig("/Users/hnamkung/Desktop/univmon-entropy.pdf")
    plt.show()
    plt.close()
