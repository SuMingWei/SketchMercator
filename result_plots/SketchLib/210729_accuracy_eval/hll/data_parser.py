...
def read_data_median(controlplane_folder_name):
    print(controlplane_folder_name)
    ...
    for fn in sorted(os.listdir(controlplane_folder_name)):
        ...
        if flag == True:
            ...
            # print(v)
            count += 1
            original_set.append(abs(v[0]-v[1])/v[0]*100)
            hw_set.append(abs(v[0]-v[3])/v[0]*100)

            if count == 5:
                original.append(median(original_set))
                hw.append(median(hw_set))
                count = 0
                original_set = []
                hw_set = []
