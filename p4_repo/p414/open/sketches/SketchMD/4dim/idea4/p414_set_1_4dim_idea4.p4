...
CM_INSTANCE_KEY_3_SIZE(4, key_four_tuple, ipv4.srcAddr, ipv4.dstAddr, tcp.seqNo, 65536)
...
control ingress {
    CM_RUN_KEY_ALL(4, ipv4.srcAddr, ipv4.dstAddr, tcp.seqNo)
    CM_RUN_KEY_1_HALF(1, ipv4.srcAddr)
    CM_RUN_KEY_1_HALF(2, ipv4.dstAddr)
    CM_RUN_KEY_2_HALF(3, ipv4.srcAddr, ipv4.dstAddr)
    CM_RUN_KEY_3_HALF(4, ipv4.srcAddr, ipv4.dstAddr, tcp.seqNo)

    if(cm_md_1.above_threshold == 1) {
        if(cm_md_2.above_threshold == 1) {
            if(cm_md_3.above_threshold == 1) {
                if(cm_md_4.above_threshold == 1) {
