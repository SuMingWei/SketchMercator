...
control SwitchIngress(
    ...
    action set_if_info(bit<1> is_ext_2) {
        ig_md.is_ext_2 = is_ext_2; // 0: internal, 1: external
    }
    table if_info {
        key = { 
    ...
    apply {
        if(hdr.ipv4.isValid() && hdr.tcp.isValid() && !meta.ipv4_checksum_err) {
            if(if_info.apply().hit) { // check whether the pkt comes from internal or external
                if(ig_md.is_ext_2 == 0){ // from internal
                    if (hdr.tcp.flag == 1) {
                        fwTcp_learn();
                    } else {
                        ipv4_lpm.apply();
