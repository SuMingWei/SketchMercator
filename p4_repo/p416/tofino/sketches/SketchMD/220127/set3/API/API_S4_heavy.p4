    ...
    inout bit<1> is_above_threshold)
{
    ...
    table subtract_table {
        ...
        const default_action = subtract_table_act;
        size = 16;
    }
    action above_threshold_table_act() {
        is_above_threshold = 1;
    }
    table above_threshold_table {
        actions = {
            above_threshold_table_act;
        }
        const default_action = above_threshold_table_act;
        size = 16;
    }
    apply {
        subtract_table.apply();
        if (diff_1 == 0) {
            above_threshold_table.apply();
        }
        else if (diff_2 == 0) {
            above_threshold_table.apply();
        }
...
control heavy_flowkey_storage_stage8 (
    inout header_t hdr,
    in bit<32> srcip,
    in bit<32> dstip,
    in bit<32> both_port,
