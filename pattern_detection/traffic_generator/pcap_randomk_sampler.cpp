...
int main(int argc, char* argv[]){
    ...
    random_device rd;
    mt19937 rng(rd());
    vector<pair<Flowkey_t, int>> vec(flow_stream.begin(), flow_stream.end());
    shuffle(vec.begin(), vec.end(), rng);
    map<Flowkey_t, int> shuffle_flow_stream(vec.begin(), vec.end());

    // get sampled flow size distribution
    int topk = stoi(argv[3]);
    int cnt = 0;
    map<int, int> flow_size_dist;
    for(auto flow : shuffle_flow_stream){
        flow_size_dist[flow.second]++;
        cnt++;
        if(topk != 0 && cnt >= topk) break;
    }

    // write fsd file 
    string file_name = argv[4];
    string output_file_name = "./sampled_fs_dist/" + file_name + ".txt";
