    ...
    for subset in partition:
        if len(subset) == 1:
            ...
            if o2a_inst1.counter_update_type == o2a_inst2.counter_update_type: 
                ...
                print_msg("should not happen at bruteforce.py, exit", 0, "red")
                exit(1)
        else:
            print_msg("should not happen at bruteforce.py, exit", 0, "red")
            exit(1)
...
def find_best_O2B(inst_list):
    ...
    print_msg("    [bruteforce find best O2B start]", debug_level, "cyan")
    ...
    for p in best_O2B_partition(strict_o2a_inst_list, new_number_list):
        ...
        i+=1
        print_msg(f"    [bruteforce find best O2B] {i} {partition_candidate}", debug_level, "cyan")
        ...
        if min > val:
            ...
            msg = f"    [bruteforce find best O2B] {i} {partition_candidate}" + " [min val] %.4f" % val + f" [answer partition] {answer_partition}"
            print_msg(msg, debug_level, "yellow")

    print_msg("    [bruteforce find best O2B end]", debug_level, "cyan")
    return min, min_o2b_inst



def bf_compute_obj_func(inst_list):
...
def bf_partition(inst_list, number_list):
    if len(number_list) == 1:
        yield [ number_list ]
        return
    ...
    for smaller in bf_partition(inst_list, number_list[1:]):
        for n, subset in enumerate(smaller):
            new_subset = [ first ] + subset
...
def o2_bruteforce_main(inst_list):
    print_msg("[o2_bruteforce_main start]", 4, "cyan")
    ...
    for i, p in enumerate(bf_partition(inst_list, number_list), 1):
        partition_candidate = sorted(p)
        msg = f"[o2_bruteforce_main] {i} {partition_candidate}"
        if i % 100000 == 0:
        ...
        for subset in partition_candidate:
            ...
            key = tuple(subset)
            if key in bf_obj_func_cache:
                val, o2inst =  bf_obj_func_cache[key]
            else:
                print_msg(f"[cache miss] compute {subset}", 17, "cyan")
                new_inst_list = get_list_using_number_list(inst_list, subset)
                val, o2inst = bf_compute_obj_func(new_inst_list)
                bf_obj_func_cache[key] = (val, o2inst)
        ...
        if min > sum:
            ...
            msg = f"[o2_bruteforce_main] {i} {partition_candidate}" + " [min val] %.4f" % val + f" [answer partition] {answer_partition}"
            print_msg(msg, 5, "yellow")

    print_answer_list_msg(answer_list, 4)
    print_msg("[o2_bruteforce_main end]", 4, "cyan")
