...
def bf_partition_o1a(inst_list, number_list):
    ...
    for smaller in bf_partition_o1a(inst_list, number_list[1:]):
...
def bf_partition_o1b(o1a_list, partition, number_list):
    if len(number_list) == 1:
        yield [ number_list ]
        return
    ...
    for smaller in bf_partition_o1b(o1a_list, partition, number_list[1:]):
        for n, subset in enumerate(smaller):
            new_subset = [ first ] + subset
            if len(new_subset) <= 3:
                yield smaller[:n] + [[ first ] + subset]  + smaller[n+1:]
...
def o1_bruteforce_main(inst_list, o2_result):
    bf_obj_func_cache = {}
    min = float('inf')
    answer_partition = []
    answer_list = []

    print_msg("[o1_bruteforce_main start]", 4, "cyan")
    ...
    for ia, pa in enumerate(bf_partition_o1a(o1a_list, number_list_o1a), 1):
        o1a_partition = sorted(pa)
        # o1a_partition = [[0, 3], [1, 2], [4], [5], [6], [7]]
        msg = f"[o1a_bruteforce_main] {ia} {o1a_partition}"
        print_msg(msg, 10, "cyan")
        # print(msg)
        number_list_o1b = list(range(len(o1a_partition)))
        for ib, pb in enumerate(bf_partition_o1b(o1a_list, o1a_partition, number_list_o1b), 1):
            o1b_partition = sorted(pb)
            msg = f"    [o1b_bruteforce_main] {ib} {o1b_partition}"
            print_msg(msg, 10, "cyan")
            ...
            if min > sum:
                ...
                msg = f"    [o1b_bruteforce_main] {ia} {o1a_partition} {ib} {o1b_partition}" + " [min val] %.4f" % min
