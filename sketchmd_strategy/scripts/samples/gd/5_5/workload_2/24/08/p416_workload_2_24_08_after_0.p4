#include <core.p4>
#if __TARGET_TOFINO__ == 2
#include <t2na.p4>
#else
#include <tna.p4>
#endif
#include "headers.p4"
#include "util.p4"
#include "metadata.p4"
struct metadata_t {
    bit<16> src_port;
    bit<16> dst_port;
    bit<32> hf_srcip;
    bit<32> hf_dstip;
    bit<16> hf_srcport;
    bit<16> hf_dstport;
    bit<8> hf_proto;
    METADATA_DIM(1)
    METADATA_DIM(2)
    METADATA_DIM(3)
    METADATA_DIM(4)
    METADATA_DIM(5)
    METADATA_DIM(6)
    METADATA_DIM(7)
    METADATA_DIM(8)
    METADATA_DIM(9)
    METADATA_DIM(10)
    METADATA_DIM(11)
    METADATA_DIM(12)
    METADATA_DIM(13)
    METADATA_DIM(14)
    METADATA_DIM(15)
    METADATA_DIM(16)
    METADATA_DIM(17)
    METADATA_DIM(18)
    METADATA_DIM(19)
    METADATA_DIM(20)
    METADATA_DIM(21)
    METADATA_DIM(22)
    METADATA_DIM(23)
    METADATA_DIM(24)
}
#include "parser.p4"
#include "all_include.p4"
#define SRCIP hdr.ipv4.src_addr
#define DSTIP hdr.ipv4.dst_addr
#define SRCPORT ig_md.src_port
#define DSTPORT ig_md.dst_port
#define PROTO hdr.ipv4.protocol
#define SIZE hdr.ipv4.total_len
INIT_HEAVY_FLOWKEY_STORAGE_CONFIG_100(32768, 15, 22528)

control SwitchIngress(
        inout header_t hdr,
        inout metadata_t ig_md,
        in ingress_intrinsic_metadata_t ig_intr_md,
        in ingress_intrinsic_metadata_from_parser_t ig_prsr_md,
        inout ingress_intrinsic_metadata_for_deparser_t ig_dprsr_md,
        inout ingress_intrinsic_metadata_for_tm_t ig_tm_md) {

    // O1 hash init
        COMPUTE_HASH_100_32_32(32w0x30244f0b) d_3_auto_sampling_hash_call;

        COMPUTE_HASH_100_32_32(32w0x30244f0b) d_5_auto_sampling_hash_call;

        COMPUTE_HASH_100_16_16(32w0x30244f0b) d_22_auto_sampling_hash_call;

        COMPUTE_HASH_100_16_16(32w0x30244f0b) d_24_auto_sampling_hash_call;

    //

    T1_INIT_3( 2, 100, 524288)
    T1_INIT_4( 1, 100, 524288)
    // apply SALUMerge; HLL (row1) - HLL (row1) 
        TCAM_LPM_HLLPCSA() d_3_tcam_lpm;
        T4_T4_KEY_UPDATE_100_65536(32w0x30243f0b) update_3_1;
    //

    // apply SALUMerge; big - CM/Kary/CS / small - Ent/PCSA 
        ABOVE_THRESHOLD_CONSTANT_3(100) d_11_above_threshold;
        COMPUTE_HASH_100_32_32(32w0x30244f0b) d_11_hash_call;
        TCAM_LPM_HLLPCSA() d_11_tcam_lpm;
        GET_BITMASK() d_11_get_bitmask;
        T2_T5_KEY_UPDATE_100_16384(32w0x30243f0b) update_11_1;
        T2_KEY_UPDATE_100_16384(32w0x30243f0b) update_11_2;
        T2_KEY_UPDATE_100_16384(32w0x30243f0b) update_11_3;
    //


    // apply O2
        T2_KEY_UPDATE_100_8192(32w0x30243f0b) update_8_1;
        ABOVE_THRESHOLD_CONSTANT_1(100) d_8_above_threshold;
    // 

    // apply SALUMerge; big - PCSA or ENT / small - ENT/CM/Kary, CS, PCSA 
        COMPUTE_HASH_100_16_16(32w0x30244f0b) d_10_res_hash_call;
        ABOVE_THRESHOLD_CONSTANT_1(100) d_13_above_threshold;
        T3_T2_KEY_UPDATE_100_16384(32w0x30243f0b) update_10_1;
        ABOVE_THRESHOLD_CONSTANT_1(100) d_14_above_threshold;
        T3_T2_KEY_UPDATE_100_16384(32w0x30243f0b) update_10_2;
        ABOVE_THRESHOLD_CONSTANT_3(100) d_16_above_threshold;
        T3_T2_KEY_UPDATE_100_16384(32w0x30243f0b) update_10_3;
        T3_T2_KEY_UPDATE_100_16384(32w0x30243f0b) update_10_4;
        T3_T2_KEY_UPDATE_100_16384(32w0x30243f0b) update_10_5;
    //

    // apply SALUMerge; big - CM/Kary/CS / small - Ent/PCSA 
        ABOVE_THRESHOLD_CONSTANT_3(100) d_9_above_threshold;
        T2_T2_KEY_UPDATE_100_8192(32w0x30243f0b) update_9_1;
        T2_T2_KEY_UPDATE_100_8192(32w0x30243f0b) update_9_2;
        T2_KEY_UPDATE_100_8192(32w0x30243f0b) update_9_3;
    //

    T2_INIT_HH_3(18, 100, 8192)

    // apply O2
        T2_KEY_UPDATE_100_8192(32w0x30243f0b) update_7_1;
        T2_KEY_UPDATE_100_8192(32w0x30243f0b) update_7_2;
        T2_KEY_UPDATE_100_8192(32w0x30243f0b) update_7_3;
        T2_KEY_UPDATE_100_8192(32w0x30243f0b) update_7_4;
        ABOVE_THRESHOLD_CONSTANT_4(100) d_7_above_threshold;
    // 

    T3_INIT_HH_4(15, 100, 16384)

    // apply SALUMerge; big - MRAC (1 row) / small - MRAC (1 row) 
        COMPUTE_HASH_100_16_16(32w0x30244f0b) d_22_sampling_hash_call;
        COMPUTE_HASH_100_16_10(32w0x30244f0b) d_22_index_hash_call_1;
        LPM_OPT_MRAC_2() d_22_tcam_lpm_2;
        T2_T2_INDEX_UPDATE_16384() update_22;
    // 

    UM_INIT_1(24, 100, 11, 32768)

    HEAVY_FLOWKEY_STORAGE_CONFIG_100(32w0x30243f0b) heavy_flowkey_storage;



    apply {
        // O1 hash run
            d_3_auto_sampling_hash_call.apply(DSTIP, ig_md.d_3_sampling_hash_32);

            d_5_auto_sampling_hash_call.apply(DSTIP, ig_md.d_5_sampling_hash_32);

            d_22_auto_sampling_hash_call.apply(DSTIP, ig_md.d_22_sampling_hash_16);

            d_24_auto_sampling_hash_call.apply(DSTIP, ig_md.d_24_sampling_hash_16);

        //

        T1_RUN_3_KEY_1( 2, DSTIP) if (ig_md.d_2_est1_1 == 0 || ig_md.d_2_est1_2 == 0 || ig_md.d_2_est1_3 == 0) { /* process_new_flow() */ }
        T1_RUN_4_KEY_1( 1, DSTIP) if (ig_md.d_1_est1_1 == 0 || ig_md.d_1_est1_2 == 0 || ig_md.d_1_est1_3 == 0 || ig_md.d_1_est1_4 == 0) { /* process_new_flow() */ }
        // apply SALUMerge; HLL (row1) - HLL (row1)  
            d_3_tcam_lpm.apply(ig_md.d_3_sampling_hash_32, ig_md.d_3_level);
            update_3_1.apply(DSTIP, ig_md.d_3_level);
        //

        // apply SALUMerge; big - CM/Kary/CS / small - Ent/PCSA 
            d_11_tcam_lpm.apply(ig_md.d_5_sampling_hash_32, ig_md.d_11_level);
            d_11_get_bitmask.apply(ig_md.d_11_level, ig_md.d_11_bitmask);
            update_11_1.apply(DSTIP, SIZE, ig_md.d_11_bitmask, ig_md.d_11_est_1);
            update_11_2.apply(DSTIP, SIZE, ig_md.d_11_est_2);
            update_11_3.apply(DSTIP, SIZE, ig_md.d_11_est_3);
            RUN_HEAVY_FLOWKEY_CONSTANT_NOIF_3(11)
        //


        // apply O2
        T2_RUN_AFTER_1_KEY_1(8, DSTIP, SIZE)
        // 

        // apply SALUMerge; big - PCSA or ENT / small - ENT/CM/Kary, CS, PCSA 
            d_10_res_hash_call.apply(DSTIP, ig_md.d_10_res_hash);
            update_10_1.apply(DSTIP, 1, ig_md.d_10_res_hash[1:1], SIZE, ig_md.d_10_est_1);
            d_13_above_threshold.apply(ig_md.d_10_est_1, ig_md.d_13_above_threshold);
            update_10_2.apply(DSTIP, 1, ig_md.d_10_res_hash[2:2], SIZE, ig_md.d_10_est_2);
            d_14_above_threshold.apply(ig_md.d_10_est_2, ig_md.d_14_above_threshold);
            update_10_3.apply(DSTIP, SIZE, ig_md.d_10_res_hash[3:3], SIZE, ig_md.d_10_est_3);
            update_10_4.apply(DSTIP, SIZE, ig_md.d_10_res_hash[4:4], SIZE, ig_md.d_10_est_4);
            update_10_5.apply(DSTIP, SIZE, ig_md.d_10_res_hash[5:5], SIZE, ig_md.d_10_est_5);
            d_16_above_threshold.apply(ig_md.d_10_est_3, ig_md.d_10_est_4, ig_md.d_10_est_5, ig_md.d_16_above_threshold);
        //

        // apply SALUMerge; big - CM/Kary/CS / small - Ent/PCSA 
            update_9_1.apply(DSTIP, SIZE, 1, ig_md.d_9_est_1);
            update_9_2.apply(DSTIP, SIZE, 1, ig_md.d_9_est_2);
            update_9_3.apply(DSTIP, SIZE, ig_md.d_9_est_3);
            RUN_HEAVY_FLOWKEY_CONSTANT_NOIF_3(9)
        //

        T2_RUN_AFTER_3_KEY_1(18, DSTIP, 1)

        // apply O2
        T2_RUN_AFTER_4_KEY_1(7, DSTIP, 1)
        // 

        T3_RUN_AFTER_4_KEY_1(15, DSTIP, SIZE)

        // apply SALUMerge; big - MRAC (1 row) / small - MRAC (1 row)
        d_22_tcam_lpm_2.apply(ig_md.d_22_sampling_hash_16, ig_md.d_22_base_16_1024, ig_md.d_22_base_16_2048);
        d_22_index_hash_call_1.apply(DSTIP, ig_md.d_22_index1_16);
        update_22.apply(ig_md.d_22_base_16_1024, ig_md.d_22_index1_16);
        // 

        // UnivMon for inst 24
            d_24_res_hash_call.apply(DSTIP, ig_md.d_24_res_hash); 
            d_24_tcam_lpm.apply(ig_md.d_24_sampling_hash_16, ig_md.d_24_base_16, ig_md.d_24_threshold); 
            d_24_index_hash_call_1.apply(DSTIP, ig_md.d_24_index1_16); 
            UM_RUN_END_1(24, ig_md.d_24_res_hash, 1) 
            RUN_HEAVY_FLOWKEY_NOIF_1(24) 
        //

        if(
        ig_md.d_11_above_threshold == 1
        || ig_md.d_8_above_threshold == 1
        || ig_md.d_13_above_threshold == 1
        || ig_md.d_14_above_threshold == 1
        || ig_md.d_16_above_threshold == 1
        || ig_md.d_9_above_threshold == 1
        || ig_md.d_18_above_threshold == 1
        || ig_md.d_7_above_threshold == 1
        || ig_md.d_15_above_threshold == 1
        || ig_md.d_24_above_threshold == 1
        ) {
            ig_md.hf_dstip = DSTIP; 
        }
        if(
        ig_md.d_11_above_threshold == 1
        || ig_md.d_8_above_threshold == 1
        || ig_md.d_13_above_threshold == 1
        || ig_md.d_14_above_threshold == 1
        || ig_md.d_16_above_threshold == 1
        || ig_md.d_9_above_threshold == 1
        || ig_md.d_18_above_threshold == 1
        || ig_md.d_7_above_threshold == 1
        || ig_md.d_15_above_threshold == 1
        || ig_md.d_24_above_threshold == 1
        ) {
            heavy_flowkey_storage.apply(ig_dprsr_md,ig_md.hf_dstip); 
        }

    }
}
struct my_egress_headers_t {}
struct my_egress_metadata_t {}
parser EgressParser(packet_in        pkt,
    out my_egress_headers_t          hdr,
    out my_egress_metadata_t         meta,
    out egress_intrinsic_metadata_t  eg_intr_md)
{
    state start {
        pkt.extract(eg_intr_md);
        transition accept;
    }
}
control EmptyEgress(
        inout my_egress_headers_t hdr,
        inout my_egress_metadata_t eg_md,
        in egress_intrinsic_metadata_t eg_intr_md,
        in egress_intrinsic_metadata_from_parser_t eg_intr_md_from_prsr,
        inout egress_intrinsic_metadata_for_deparser_t ig_intr_dprs_md,
        inout egress_intrinsic_metadata_for_output_port_t eg_intr_oport_md) {
    apply {}
}
control EgressDeparser(packet_out pkt,
    inout my_egress_headers_t                       hdr,
    in    my_egress_metadata_t                      meta,
    in    egress_intrinsic_metadata_for_deparser_t  eg_dprsr_md)
{
    apply {
        pkt.emit(hdr);
    }
}
Pipeline(
    SwitchIngressParser(),
    SwitchIngress(),
    SwitchIngressDeparser(),
    EgressParser(),
    EmptyEgress(),
    EgressDeparser()
) pipe;
Switch(pipe) main;