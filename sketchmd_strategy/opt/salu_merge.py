...
def get_O2B_cluster(o2a_merged_list):
    ...
    return clusters
        ...
        # sum = 0
        # for o2a_inst in self.small_o2a_inst_list:
        #     sum += o2a_inst.get_max_r()
        # data["O1"] = [before["hashcall"] - after["hashcall"] - df["O2"]["hashcall"] - global_objective_function(sum, 0, 0, 0), \
        #                 df["O1"]["salu"], \
        #                 df["O1"]["sram"]]

        # data["O2"] = [df["O2"]["hashcall"], df["O2"]["salu"], df["O2"]["sram"]]

        # data["O3"] = [global_objective_function(sum, 0, 0, 0),
